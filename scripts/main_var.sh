...
unique_id=$(date +'%m-%d_%H:%M_testingPhi')
...
if [ "$decentralized" = true ] && [ "$exact_diffusion" = false ] && [ "$A_avg" = true ]; then
  python3 src/main.py --decentralized --average_with_A --edge_prob $edge_prob --model=resnet --gpu=0 --iid=1 --batch_size=256 --local_bs=256 --local_ep=$local_ep --epochs=$epochs --log_file_name "${output_dir}/skew_ssl_comm" \
...
elif [ "$decentralized" = true ] && [ "$exact_diffusion" = true ]; then
  ...
  python3 src/main.py --model=resnet --gpu=0 --iid=1 --batch_size=256 --local_bs=256 --local_ep=$local_ep --epochs=$epochs --log_file_name "${output_dir}/skew_ssl_comm" \
                      --lr=0.001 --optimizer=adam --backbone=resnet18 --num_users=$num_users --frac=1 --y_partition_skew --y_partition_ratio=0 --log_directory "$output_dir" > "${output_dir}/output_log.txt" 
fi
