...
output_dir="agentsVar/${job_name}"
mkdir -p $output_dir

time python3 src/main.py --decentralized --edge_prob $edge_prob --model=resnet --gpu=1 --iid=1 --batch_size=256 --local_bs=256 --local_ep=$local_ep --epochs=$epochs --log_file_name "comm_scripts/best_linear_statistics_skew_ssl_comm.csv" \
...
time python3 src/main.py --exact_diffusion --decentralized --edge_prob $edge_prob --model=resnet --gpu=1 --iid=1 --batch_size=256 --local_bs=256 --local_ep=$local_ep --epochs=$epochs --log_file_name "comm_scripts/best_linear_statistics_skew_ssl_comm.csv" \
                      --lr=$lr_exp --optimizer=adam --backbone=resnet18 --num_users=$num_users --log_directory "$output_dir" > "${output_dir}/output_log.txt" 2> "${output_dir}/error.txt"
...
edge_prob=1
unique_id=$(date +'%m-%d_%H:%M')
job_name="${unique_id}_Avereage_A_drich-${dirichlet_beta}_dec-${decentralized}_ED-${exact_diffusion}_ep${edge_prob}_e${epochs}_le${local_ep}_a${num_users}"
output_dir="lr_exp/${job_name}"
mkdir -p $output_dir
