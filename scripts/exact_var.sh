...
if [ "$decentralized" = true ] && [ "$exact_diffusion" = true ]; then
  python3 src/main.py --decentralized --exact_diffusion --edge_prob $edge_prob --model=resnet --gpu=1 --iid=1 --batch_size=256 --local_bs=256 --local_ep=$local_ep --epochs=$epochs --log_file_name "${output_dir}/skew_ssl_comm" \
